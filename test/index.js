...
describe("Server", () => {
  ...
  beforeEach(() => {
    fs.emptyDirSync(config.get('FILES_ROOT'))
  })
  ...
  describe("GET /file.ext", () => {
    context("When exists", () => {
      beforeEach(() => {
        ...
        fs.copySync(`${fixturesRoot}/small.png`, config.get('FILES_ROOT') + '/small.png')
      })
    ...
    context("otherwise", () => {
      it("returns 404", done => {
        request.get(`${host}/small.png`, (error, response) => {
          ...
          // debugger
          response.statusCode.should.be.equal(404) // Number.prototype.should = {}
          done()
        })
      })
    })
